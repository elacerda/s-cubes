...
class control:
    ...
    def _correct_dirpath(self, path):
        self.data_dir = join(path, self.data_dir)
        self.zpcorr_dir = join(path, self.data_dir, self.zpcorr_dir)
        self.zp_table = join(path, self.data_dir, self.zp_table)
        self.output_dir = join(path, self.work_dir, self.galaxy)
        self.path = path
        print_level(f'path: {self.path}', 2, self.verbose)
